...
namespace Pitstop.NotificationService.Controller
{
    ...
    public class NotificationHandlerController : ControllerBase
    {
        ...
        public NotificationHandlerController(INotificationRepository repo,
        ...
        public async Task HandleAsync(DayHasPassed dhp)
        {
            ...
            foreach (var jobsPerCustomer in jobsToNotify.GroupBy(job => job.CustomerId))
            {
                ...
                {
                    body.AppendLine(
                        $"- {job.StartTime.ToString("dd-MM-yyyy")} at {job.StartTime.ToString("HH:mm")} : " +
